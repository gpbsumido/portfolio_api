...
def load_race_results(year, event_round):
    ...
    try:
        ...
        logging.error(
            f"Race session load failed for round {event_round}. Details: {str(e)}"
        )
        return None
...
def get_championship_points(year, round_number=None, points_type="driver"):
    try:
        ...
        print(
            f"DEBUG: Args received - year={year}, round={round_number}, type={points_type}",
            file=sys.stderr,
        )
        ...
        if points_type == "per_race":
            ...
            return json.dumps(
                {"year": year, "points_type": points_type, "results": results_per_race},
                indent=4,
                default=safe_json,
            )
        ...
        for _, event in event_schedule.iterrows():
            ...
            for _, driver in race_results.iterrows():
                if points_type == "driver":
                    ...
                    raise ValueError(
                        f"Invalid points type: {points_type}. Must be 'driver' or 'constructor'."
                    )
        ...
        if round_number is None:
            ...
            return json.dumps(
                {"year": year, "points_type": points_type, "results": results},
                indent=4,
                default=safe_json,
            )
        ...
        if round_number not in event_schedule["RoundNumber"].values:
            raise ValueError(
                f"No race found with round number '{round_number}' in {year}."
            )
        ...
        if points_type == "driver":
            position_col = (
                "PositionText" if "PositionText" in race_results.columns else "Position"
            )
            championship_points = [
                {
                    "driver": safe_get(driver, "Abbreviation"),
                    ...
                    "dnf": not safe_get(driver, "Status", "")
                    .lower()
                    .startswith("finished"),
                    "points": safe_get(driver, "Points", 0),
                }
            ...
            for _, driver in race_results.iterrows():
                ...
                constructors[constructor] = constructors.get(constructor, 0) + safe_get(
                    driver, "Points", 0
                )
            championship_points = [
