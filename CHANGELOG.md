...
- **Deterministic on purpose, which ruled two things out.** The measurement pins `supportedArchitectures` to linux/x64 — sharp resolves a different prebuilt libvips per platform, an 18 MiB swing between my Mac and the deploy target that has nothing to do with any change under review — and runs with `--ignore-scripts`, so ffmpeg-static's network download is excluded and the figure is a known understatement of the deployed image rather than a number that moves with whoever ran it. Boot wall-clock is not gated at all: importing the built app takes 268-294 ms on an idle laptop, and that spread is far wider on a shared runner. A guard that flakes gets ignored, so that one stays a hand measurement.
...
- **The one number worth staring at**: `ffprobe-static` is 335 MiB of the 442, because it ships prebuilt binaries for every platform in its tarball. Three quarters of this budget is a single dependency, and replacing it is the only change that would meaningfully move the figure.
