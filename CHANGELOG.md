...
- **Serve recent past fixtures on the events endpoint.** `GET /api/zeroproof/events` returns upcoming games only; a new opt-in `?include=past` also returns recently-finished fixtures (kickoff within the last 7 days) with their last lines, so the frontend board can offer a "show past fixtures" toggle. Read-only and bounded: a new `listPastEventsWithLines(windowDays)` reuses the same latest-snapshot-per-market join as the upcoming query — extracted into a shared `attachLatestLines` helper, with no behaviour change to the upcoming path — and `service.listEvents({ includePast })` appends the past set only when the param is present. Covered by endpoint tests: `?include=past` returns both the upcoming and past sets, and the default never touches the past query.
...
- **Odds come from ESPN's win probability, or the projection gap.** A matchup is priced off ESPN's published per-side win probability when it has one, else off the projected-points margin (a logistic on the gap), instead of a flat pick'em. Fair odds, no vig.
- **Bets on those invalid pre-season matchups are voided and refunded.** A league that isn't open for betting shouldn't carry open bets — and since a league only ever goes from closed to open, never back, any open bet on a closed league is a pre-open one that shouldn't count. The sync now voids and refunds them: the stake comes back and the bet stays out of the win/loss record. Idempotent, so a re-run refunds nothing twice.
- **The god's view now shows every bettor's email.** ZeroProof routes didn't run the `upsertUser` middleware every other authenticated module uses, so a user who only placed bets never had their verified email captured into `users` — the admin bets view had only a handle to show. The authenticated ZeroProof routes now upsert the caller, so an email lands the next time they act (there's no server-side backfill; the email only arrives in their token).

## 2026-09-15 - version 5.21.1

- **Quieted the IPv6 rate-limit warning the check-in module logged on every boot.** The arrival limiter keys by Auth0 sub with an IP fallback, but it used `req.ip` raw. express-rate-limit v8 flags that as `ERR_ERL_KEY_GEN_IPV6` and its `ConsoleLogger` prints the flagged error with a full stack on startup, which reads like a crash even though the server keeps running. It's also a real bypass: an IPv6 caller is handed a whole /64, so keying on the full address lets one person mint unlimited buckets by varying the low bits. The IP fallback now goes through the `ipKeyGenerator` helper — v4 unchanged, v6 collapsed to its /64 — matching what the operator module already does. Extracted the key logic to `submitRateLimitKeyOf` and covered it: sub priority, IPv6 normalization, IPv4 passthrough, and v8's exact guard heuristic.

## 2026-09-15 - version 5.21.0

- **A budgets domain, so the paul-explore budget tracker can be shared for real.** The tracker shipped browser-only behind a storage seam; this is the backend it was shaped for. New `/api/budgets` module (migration `044_budgets`) with five tables — `budgets`, `budget_people`, `budget_members`, `budget_expenses`, `budget_join_requests`. A signed-in user gets one budget provisioned on first read, can log/edit/delete expenses (with tags, per-person splits, an optional note and vendor), and add people to attribute spend to. Making a budget public records the owner's verified email; anyone can then ask to join it by that email, and the owner approves or denies — approval adds them as a member and a person in one transaction. Every mutation checks owner-or-member access first, and a private/missing budget both answer 404 so the public flag can't be used to probe which emails have a budget. Discovery resolves against the `users` table on the verified namespaced email claim, same as calendar sharing. Repository SQL and route permissions are unit-tested against the mocked pool (the real-DB smoke path and the migration run under CI's Postgres).

- **Admin god's view of every ZeroProof bet.** New `GET /api/zeroproof/admin/bets` returns every user's bets — live and resolved — newest first, so I can see and search the whole book instead of only my own history. It's admin-gated with the same `checkJwt` + `requireAdmin` (`ADMIN_ALLOWED_EMAILS`) gate `/house` uses; a signed-in non-admin gets a 403. Each row carries the normal bet DTO plus `userSub`, `email`, `handle` (the profile's display name, falling back to the username, or null when there's no profile yet) and the wallet's `mode`, so league play is distinguishable from season/challenge. It joins bets → wallets → users, left-joining users and profiles so a bet never drops out just because its bettor hasn't set up a profile. `?q=` filters case-insensitively across email, username, display name, selection and market. Records (W-L-P, counts) are derived on the frontend by grouping these rows, so there's no second endpoint. Covered by tests: the endpoint maps identity/handle/mode to ISO-dated DTOs (handle falling back to the username), and forwards `?q=` to the repository. Scale note: returns all bets unpaginated, fine at current volume.

## 2026-09-11 - version 5.19.1

- **ZeroProof bets on finished games now settle.** The results provider fetched The Odds API `/scores` with `daysFrom=1`, so any game that finished more than ~1 day before a settle run was never in the response — its event stayed `upcoming` with no result and the open bets sat "open" forever. (Found it in prod: two events, commenced a day and two days earlier, both still `upcoming` with `result` null.) The window now defaults to `3` — The Odds API's maximum — so a game that finished up to three days ago still gets a result and grades, and it's tunable per deployment with `ZEROPROOF_RESULTS_DAYS_FROM` (missing, empty, or out-of-range values clamp to 1-3 and fall back to 3). Covered by a unit test on the window parser and one asserting the default fetch uses a 3-day window. Note the hard ceiling: The Odds API `/scores` won't return games older than three days, so a game that ages past that before a settle run has to be graded by hand.

## 2026-09-10 - version 5.19.0

- **Error messages now actually reach the user.** The frontend reads the JSON `error` field and shows it in a toast, but the central error handler had been putting the error's class name there and stranding the real sentence in `message` — so every `throw new ConflictError('You already have an active season wallet')` surfaced to a person as the word "ConflictError", and a `NotFoundError` as "NotFoundError". That was true for the overwhelming majority of endpoints, which throw an `AppError`. The fix is one shape, answered everywhere: `errorHandler`, the `validate` middleware, and the 404 catch-all all return `{ error, message, code, statusCode, details? }`, where `error` and `message` both carry the human sentence (different screens read different keys) and the new `code` carries the machine name ("NotFoundError", "ValidationError"). A non-breaking superset — both frontend readers get a real message now, and there's a stable code for anything programmatic. The `validate` middleware also leads with a readable line ("email: must be a valid email") instead of a content-free "Validation failed", and the `AppError` subclass defaults became plain sentences ("You don't have permission to do that.") since a bare throw is now shown verbatim. Covered by rewritten `errorHandler` and new `validate` tests pinning the shape, plus the profiles/vitals integration tests updated to it.
- **Stopped a few messages leaking internals.** Image processing failures returned Sharp's raw error text to the uploader; that's now a friendly "Couldn't process that image. Please try a different file." with the real cause logged instead. The unknown-error 500 no longer echoes raw `err.message` shapes, the multer fallback reads plainly, and the admin-token guard on saving NBA results explains what it wants rather than a bare "Unauthorized". Full test suite green (713 passing), typecheck and lint clean.
- **One phrasing for "you're not signed in".** The same 401 gate said five different things across modules — "Not signed in", "Missing user.", "Unauthorized: No user sub found", "Missing user identity". They all now fall through to the `UnauthorizedError` default ("You need to be signed in to do that."), so an unauthenticated request reads the same everywhere. Feedback's two "not found" wordings are unified too (the internal sentinel string the update path matches on is left as-is).
- **Consistent casing.** A handful of modules started their messages lowercase — "store not found", "slug already taken", "flag not found", "adjustment not found", plus a few service-token responses — while everywhere else capitalized. Those entity/status messages are now capitalized to match. The field-leading validation messages ("email is required", "value must be a number") are deliberately left as they are, since the lowercase word is the actual request field. Only trailing-period style is still mixed across modules; that's pure cosmetics and left alone.

## 2026-09-09 - version 5.18.0

- **No betting a game that's already started.** Placing a bet now checks the event first and 409s once it's under way, so a stale board can't sneak a bet onto a live game. The rule is uniform — `isEventBettable` in `placement.ts`: the event must still be `'upcoming'` and its `commenceTime` must be in the future. For real fixtures that start time is the gate. ESPN fantasy matchups carry a synthetic commence time that always reads as future (there's no real kickoff for a week-long matchup), so the ESPN sync now flags a matchup as `started` the moment either side has points on the board, and `upsertEvent` records that as `status = 'started'`. The existing board query (`status = 'upcoming' AND commence_time > now()`) then drops it automatically, and the placement gate rejects it — no migration, the `status` column already exists. `upsertEvent` only ever pushes status forward to `started`, never back to `upcoming`, so it can't reopen a started matchup or undo a settlement's `final`. Covered by tests: `isEventBettable` unit cases, `normalizeMatchups` setting `started` off points, and integration tests that a started real fixture and a started ESPN matchup both 409 without touching the ledger.
...
- **A league is a scope over the machinery that already exists.** Each member bets from a league-scoped wallet (`mode='league'`, tagged with `league_id`), so placement, the ledger, and settlement are unchanged — a league only decides which wallet you bet from and who your standing compares against. League bankrolls are simulated: seeded from the house with a deposit ledger pair, no locked principal and no refund, so the unlock cron leaves them alone.
- **League play stays out of the global record.** The sharp/ROI leaderboard and your profile stats scan season and challenge wallets only, so a for-fun contest can't inflate the record that's the actual product. Standings rank on balance (the contest metric), ROI as the tiebreak.
- **Joining is idempotent and betting is scoped safely.** A unique `(league_id, user_sub)` means a double-join returns the same wallet rather than a second one, and a partial unique index lets a user hold an active wallet in many leagues at once while season/challenge stay one-per-mode. All the rules (target-above-bankroll, future deadline, joinable-when-open-and-not-full, ranking) are pure and unit-tested, and every endpoint is covered end to end against a mocked repository. Winner settlement (threshold + timeline) lands in the next slice.
