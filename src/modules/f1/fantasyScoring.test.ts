...
const { calculateQualifyingPoints, calculateRacePoints } = require('../../../utils/fantasyScoring') as {
  calculateQualifyingPoints: (result: Record<string, unknown>) => { points: number; breakdown: Record<string, number> };
  calculateRacePoints: (race: Record<string, unknown>, quali: Record<string, unknown>) => { points: number; breakdown: Record<string, number> };
};
...
describe('calculateRacePoints', () => {
  ...
  test('driver of the day adds 10 points', () => {
    ...
    expect(result.points).toBe(15 + 10);
  });
